...
export const updateToolImagesController = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (removeImageIds.length > 0) {
      ...
      if (imagesToRemove.length !== removeImageIds.length) {
        ...
        const unauthorizedIds = removeImageIds.filter(
          (id) => !foundIds.has(id)
        );
        ...
        return errorResponse(
          ...
          `One or more image IDs (${unauthorizedIds.join(
            ", "
          )}) do not belong to tool ID ${listing_id} or do not exist.`,
      ...
      if (
        !potentialPrimaryImage &&
        !new_primary_id &&
        !isNewFilePrimaryRequested
      ) {
        return errorResponse(
          res,
          "Deletion Blocked",
